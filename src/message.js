	...
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				...
				try {
					profilePic = await naze.profilePictureUrl(n, 'image');
				} catch {
					profilePic = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}

				try {
					userName = await naze.getName(n);
				} catch {
					userName = 'Pengguna';
				}
				...
				if (action === 'add') {
					...
					thumbUrl = `https://api.siputzx.my.id/api/canvas/welcomev2?username=${encodeURIComponent(userName)}&guildName=${encodeURIComponent(metadata.subject)}&memberCount=${metadata.participants.length}&avatar=${encodeURIComponent(profilePic)}&background=https://pomf2.lain.la/f/9egnxuu.jpg`;

				} else if (action === 'remove') {
					if (db.groups[id].leave) {
					...
					thumbUrl = `https://api.siputzx.my.id/api/canvas/goodbyev2?username=${encodeURIComponent(userName)}&guildName=${encodeURIComponent(metadata.subject)}&memberCount=${metadata.participants.length}&avatar=${encodeURIComponent(profilePic)}&background=https://pomf2.lain.la/f/9egnxuu.jpg`;

				} else if (action === 'promote') {
					if (db.groups[id].promote) {
						messageText = db.groups[id]?.text?.setpromote || `@\nPromoted in ${metadata.subject}\nBy @admin`;
					}
					updateAdminStatus(participants, metadata.participants, 'admin');
					thumbUrl = profilePic;

				} else if (action === 'demote') {
					if (db.groups[id].demote) {
						messageText = db.groups[id]?.text?.setdemote || `@\nDemoted in ${metadata.subject}\nBy @admin`;
					}
				...
				if (messageText && naze.public) {
					await naze.sendMessage(id, {
						text: messageText
							...
							.replace('@admin', author ? `@${author.split('@')[0]}` : '@admin')
							.replace(/(?<=\s|^)@(?!\w)/g, `@${n.split('@')[0]}`),
						contextInfo: {
							mentionedJid: [n, author],
							externalAdReply: {
								title:
									action === 'add' ? '👋 Selamat Datang' :
									action === 'remove' ? '👋 Selamat Tinggal' :
		...
		console.error('[ERROR GroupParticipantsUpdate]', e);
	}
}
